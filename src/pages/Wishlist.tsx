  ...
  return (
    <div className="container-custom py-10">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={handleAddAllToCart} className="btn-secondary flex items-center gap-2">
          <ShoppingBag size={16} />
          Add All to Cart
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {wishlistItems.map(product => (
          <div key={product.id} className="group relative bg-white border border-pink-100 rounded-xl overflow-hidden flex flex-col h-full transition-all duration-300 hover:shadow-2xl">
            <Link to={`/product/${product.id}`} className="relative aspect-3/4 overflow-hidden block">
              ...
              {product.offerPercent > 0 && (
                <span className="absolute top-3 left-3 bg-sale text-white text-xs font-bold px-2 py-1 rounded-sm uppercase tracking-wider">
              ...
              >
                <Trash2 size={16} className="text-sale" />
              </button>
            </Link>
            <div className="p-5 flex flex-col grow">
              <span className="text-xs font-bold text-accent uppercase tracking-widest mb-1">{product.collection}</span>
              <Link to={`/product/${product.id}`} className="grow">
                <h3 className="font-heading text-base font-medium text-text-primary leading-tight line-clamp-2 mb-2 group-hover:text-accent transition-colors">
                  {product.name}
                </h3>
              </Link>
